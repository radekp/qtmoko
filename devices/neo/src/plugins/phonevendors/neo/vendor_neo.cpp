...
void NeoVibrateAccessory::setVibrateNow( const bool value, int timeoutMs, int strength )
{
    ...
    if ( value ) { //turn on
        qWriteFile("/sys/class/leds/neo1973:vibrator/trigger", "timer");
        qWriteFile("/sys/class/leds/neo1973:vibrator/delay_on", "500");
        qWriteFile("/sys/class/leds/neo1973:vibrator/delay_off", "1000");
    } else { //turn off
        qWriteFile("/sys/class/leds/neo1973:vibrator/trigger", "none");
    }
